...
function renderBody(template: TemplateKey, vars: Record<string, string>): string {
  if (template === "POLICY_NEW") {
    return [
      ...
      "",
      "※ 본 메시지는 고객님께서 keepioo 마이페이지에서 직접 등록하신 맞춤 알림 조건에 해당하는 정책이 등록될 때 발송되는 정보성 메시지입니다. 수신을 원하지 않으실 경우 마이페이지 > 알림 설정에서 언제든 해지 가능합니다.",
    ].join("\n");
  }
  // POLICY_NEW_V3
  return [
    ...
    "",
    "※ 본 메시지는 고객님께서 keepioo 마이페이지에서 직접 등록하신 맞춤 알림 조건과 사장님 가게 정보로 자동 매칭된 정책에 대해 발송되는 정보성 메시지입니다. 수신을 원하지 않으실 경우 마이페이지 > 알림 설정에서 언제든 해지 가능합니다.",
  ].join("\n");
}

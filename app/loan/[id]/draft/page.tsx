// app/loan/[id]/draft/page.tsx
// Pro 사용자 전용 — loan 정책 신청서 초안.
// welfare 와 동일 패턴 — 자영업자 wedge 의 핵심 진입 (정책자금 신청서).
...
export default async function LoanDraftPage({ params }: Props) {
  const { id } = await params;
  const supabase = await createClient();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) redirect(`/login?next=/loan/${id}/draft`);
  ...
  return <ApplicationDraftView draft={draft} programType="loan" programId={id} />;
}

...
const RETIRED_BOOTSTRAP_SEARCH_CONSOLE_ENV_ROUTE =
  "/api/admin/bootstrap-search-console-env";
...
export async function proxy(request: NextRequest) {
  if (request.nextUrl.pathname === RETIRED_BOOTSTRAP_SEARCH_CONSOLE_ENV_ROUTE) {
    return NextResponse.json(
      { error: "gone", route: "bootstrap-search-console-env retired" },
      { status: 410 },
    );
  }

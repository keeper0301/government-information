...
// 2026-05-19 (낮) — 임계 10000 → 9000 임시 하향 (commit a22e304, Solapi UI/API lag 대응).
// 2026-05-19 (저녁) — 9000 → 10000 원복 (사장님 G 승인). lag 자연 해소 가정.
//   Solapi UI 잔액 10,017원 정상 표시. API 측 lag 가 자연 해소되면 cron 결과 동기화 예상.
//   lag 미해소 동안 alert 다시 발화하지만 24h dedupe 로 사장님 발송 0 (UI amber 표시만).
// 1만원 = SMS ~220건 = 4~5일 buffer. cron 24h 1회 → 한 cron 사이 1만원→0원 추락 방지.
//

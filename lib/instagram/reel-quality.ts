...
const FALLBACK_RE = /지역·나이·소득 기준|공고마다 달라|공식 누리집이나 담당 기관|자격·마감은 바뀔 수/;
...
export function scoreReelCandidate(post: ReelVideoPostInput): ReelQualityResult {
  ...
  return {
    approved: score >= 6 && metrics.hasTarget && metrics.hasBenefit && metrics.hasApply && metrics.fallbackLikeSlides < 2,
    score,
    reasons,
    metrics,
  };

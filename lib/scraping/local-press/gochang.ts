// ============================================================
// 전북 고창군청 보도자료 수집 (2026-07-22) — 전북권 확장
// ============================================================
// 공식 보도자료: /board/list.gochang?boardId=BBS_0000179&menuCd=DOM_000000102002001000
// 목록: div.news_list li + /board/view.gochang dataSid detail link
...
const BASE_URL = "https://www.gochang.go.kr";
const BOARD_ID = "BBS_0000179";
const MENU_CD = "DOM_000000102002001000";
const LIST_URL = `${BASE_URL}/board/list.gochang?boardId=${BOARD_ID}&menuCd=${MENU_CD}`;

const ITEM_REGEX = /<li\b[^>]*>[\s\S]*?<a\b[^>]*href=["']([^"']*\/board\/view\.gochang\?[^"']*\bboardId=BBS_0000179[^"']*\bdataSid=(\d+)[^"']*)["'][^>]*>([\s\S]*?)<\/a>\s*<\/li>/gi;
...
const DETAIL_DATE_REGEX = /<strong>\s*작성일\s*<\/strong>\s*:\s*(\d{4})[.-]\s*(\d{2})[.-]\s*(\d{2})/i;
const DETAIL_BODY_REGEX = /<div\b[^>]*class=["'][^"']*\bbbs_con\b[^"']*["'][^>]*>([\s\S]*?)<\/div>\s*<\/div>\s*<p\b[^>]*class=["'][^"']*\bbbs_btn\b/i;
...
export const { scrapeAndInsert: scrapeGochangAndInsert } = createPressCollector({
  cityName: "전북 고창군",
  region: "전북",
  ministry: "전북 고창군청",
  sourceOutlet: "전북 고창군청",
  sourceCode: "local-press-gochang",
  listUrl: LIST_URL,

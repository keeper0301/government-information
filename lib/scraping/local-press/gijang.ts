// ============================================================
// 부산 기장군청 보도자료 수집 (2026-05-27) — 부산 자치구 batch
// ============================================================
// 기장군 인구 16만 (부산 16 자치구 중 유일한 '군'). 같은 SI CMS.
...
const BASE_URL = "https://www.gijang.go.kr";
const LIST_URL =
  "https://www.gijang.go.kr/board/list.gijang?boardId=BBS_0000001&menuCd=DOM_000000103001005000";

const LIST_ITEM_REGEX =
  /<a[^>]*href="(\/board\/view\.gijang\?[^"]*boardId=BBS_0000001[^"]*dataSid=(\d+)[^"]*)"[^>]*>([\s\S]{0,500}?)<\/a>/g;
...
export function parseListPage(html: string): PressNewsItem[] {
  ...
  while ((m = itemRe.exec(html)) !== null) {
    ...
    items.push({ seq, title, publishedDate, sourceUrl: fullUrl });
  }
  return items;
}
...
export const { scrapeAndInsert: scrapeGijangAndInsert } = createPressCollector(
  {
    cityName: "기장군",
    region: "부산",
    ministry: "기장군청",

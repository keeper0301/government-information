// ============================================================
// 울산 남구 보도자료 수집 (2026-07-27)
// ============================================================
// 공식 보도자료: /cop/bbs/selectBoardList.do?bbsId=press
// 목록: eGov basic_table rows + nttId detail link
...
const BASE_URL = "https://www.ulsannamgu.go.kr";
const BBS_ID = "press";
...
export function parseListItems(html: string): PressNewsItem[] {
  ...
  while ((rowMatch = rowRe.exec(html)) !== null) {
    ...
    const linkMatch = /<a\b[^>]*href=["']([^"']*selectBoardArticle\.do[^"']*\bnttId=(\d+)[^"']*)["'][^>]*>([\s\S]*?)<\/a>/i.exec(row);
    ...
    const date = stripTags(/<td\b[^>]*class=["'][^"']*\bdate\b[^"']*["'][^>]*>([\s\S]*?)<\/td>/i.exec(row)?.[1] ?? "");
...
export function parseDetailBody(html: string): string | null {
  ...
  const title = stripTags(/<div\b[^>]*class=["'][^"']*\bbbs_detail_tit\b[^"']*["'][^>]*>[\s\S]*?<h2\b[^>]*>[\s\S]*?<strong\b[^>]*>([\s\S]*?)<\/strong>[\s\S]*?<\/h2>/i.exec(detail)?.[1] ?? "");
  const meta = stripTags(/<div\b[^>]*class=["'][^"']*\bbbs_detail_tit\b[^"']*["'][^>]*>([\s\S]*?)(?:<\/div>\s*<!--\s*첨부파일|<div\b[^>]*class=["'][^"']*\bbbs_detail_content\b)/i.exec(detail)?.[1] ?? "");
  const content = stripTags(extractDivByClass(detail, "bbs_detail_content") ?? "");
...
const collector = createPressCollector({
  ...
  ministry: "울산 남구청",
  sourceOutlet: "울산 남구청",
  sourceCode: "local-press-namgu-ulsan",
  listUrl: LIST_URL,
  parseListItems,
  parseDetailBody,
});

export const scrapeNamguUlsanAndInsert = collector.scrapeAndInsert;

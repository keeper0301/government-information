// ============================================================
// 충남 홍성군청 보도자료 수집 (2026-07-21) — 충남권 확장
// ============================================================
// 공식 보도자료: /bbs/BBSMSTR_000000000842/list.do
// 목록: board--card--list 카드, fn_search_detail('{nttId}')
// 상세: /bbs/BBSMSTR_000000000842/view.do?nttId={id}
...
const BASE_URL = "https://www.hongseong.go.kr";
const BOARD_ID = "BBSMSTR_000000000842";
const LIST_URL = `${BASE_URL}/bbs/${BOARD_ID}/list.do`;

const CARD_REGEX = /<div\b[^>]*class=["'][^"']*\bcol\b[^"']*["'][^>]*>([\s\S]*?)(?=<div\b[^>]*class=["'][^"']*\bcol\b[^"']*["'][^>]*>|<div\b[^>]*class=["'][^"']*\bprogram--paging\b|<nav\b|$)/gi;
const ID_REGEX = /fn_search_detail\(['"]([^'"]+)['"]\)/i;
const TITLE_REGEX = /<strong\b[^>]*class=["'][^"']*\btit\b[^"']*["'][^>]*>([\s\S]*?)<\/strong>/i;
const LIST_DATE_REGEX = /<li\b[^>]*class=["'][^"']*\bregDate\b[^"']*["'][^>]*>[\s\S]*?(\d{4})-(\d{2})-(\d{2})/i;
const DETAIL_TITLE_REGEX = /<meta\b[^>]*property=["']og:title["'][^>]*content=["']([^"']+)["'][^>]*>/i;
const DETAIL_DATE_REGEX = /<span\b[^>]*class=["'][^"']*\bdate\b[^"']*["'][^>]*>[\s\S]*?<i>\s*등록일\s*<\/i>\s*(\d{4})-(\d{2})-(\d{2})[\s\S]*?<\/span>/i;
...
export function parseListPage(html: string): PressNewsItem[] {
  ...
  while ((match = cardRe.exec(html)) !== null) {
    ...
    const id = ID_REGEX.exec(cardHtml)?.[1];
    ...
    const dateMatch = LIST_DATE_REGEX.exec(cardHtml);
...
export const { scrapeAndInsert: scrapeHongseongAndInsert } =
  createPressCollector({
    cityName: "충남 홍성군",
    region: "충남",
    ministry: "충남 홍성군청",
    sourceOutlet: "충남 홍성군청",
    sourceCode: "local-press-hongseong",
    listUrl: LIST_URL,

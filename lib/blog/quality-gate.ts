...
const TEMPLATE_SMELL_RE = [
  // Caption 발행면에서는 `validateCaption`이 `여러분`을 계속 엄격 차단한다.
  // 여기서는 블로그 본문 전체를 보는 1차 후보 게이트라, 본문 중간의
  // `여러분의 상황` 같은 설명형 표현까지 막으면 정상 후보 20건이 FIFO로
  // 전부 막히는 오탐이 생긴다. 실제 외부 caption에 들어갈 수 있는
  // 저품질 호칭/감사/응원 문구만 본문 게이트에서 차단한다.
  /감사드립니다|함께 해주셔서|응원해주세요/,

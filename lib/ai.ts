...
4. **H1/H2/H3 계층과 글자 크기 전제 (SEO/AEO + 네이버 시각 구조)**:
   - H1 은 JSON 의 title 이 네이버 제목 필드로 들어가며, 본문 content 안에 <h1> 을 반복하지 않는다.
   - H2 는 큰 구간 제목, H3 는 H2 안의 세부 조건으로만 사용한다.
   - 네이버 발행 formatter 는 참고글(https://blog.naver.com/leclerc23/224311229716)처럼 H2=인용박스/굵은 질문 제목, H3=작은 좌측바 제목으로 다르게 보이게 한다.
...
7. **구조 · 가독성**: H2 5~6개 + 각 H2 밑 H3 0~2개 + 문단당 2~4줄.
   네이버 외부 발행까지 고려해 참고글(https://blog.naver.com/leclerc23/224311229716)의 단순한 글 흐름과 시각 리듬을 우선합니다.
   - 첫 화면: 네이버 제목(H1) 아래 짧은 정의/요약 문단 2개 → 가운데 빨간 밑줄 CTA → 👇👇 → keepioo 본사이트 해당 글 외부링크 → 상세 섹션
   - H2: 질문형 한 줄 제목. formatter 에서 인용박스/blockquote 느낌의 굵은 제목으로 보이므로 길게 쓰지 말 것
   - H3: H2 안의 세부 조건. formatter 에서 작은 좌측바 제목으로 보이므로 H2와 같은 무게로 쓰지 말 것
   - 특수효과는 정보 보조용만 사용: 가운데 빨간 CTA, 👇👇, 파란 헤더 표, 얇은 구분선, FAQ Q/A, 체크/번호 리스트
   - 문단은 좌측 정렬 기준으로 16px 전후, 넓은 행간, 짧은 단락. 과한 카드형 CTA·광고 문구보다 정보 문단 우선
   - 자동 생성 티 나는 "검색 핵심 정보", "요약 답변", "한눈에 보는 핵심", "신청 전 체크포인트" 같은 반복 제목 남발 금지

...
function buildPrompt() {
  return [
    ...
    "인스타그램은 정부정보(gov-info) legacy 3장 카드 파이프라인을 기준으로 관리해라: app/api/cron/instagram-publish/route.ts, app/api/instagram-card/[slug]/[index]/route.tsx, lib/instagram/publish.ts.",
    "keepio_agent sidecar 8장 카드 렌더러를 기준 디자인으로 삼지 마라. 기존 keepioo_official 글과 맞출 때는 Next ImageResponse/Satori 3장 카드가 canonical이다.",
    "결제, 환불, 권한 변경, 비밀값 변경, 운영 데이터 삭제, git push, 실제 인스타 수동 발행은 하지 마라.",
